...
const PageTabNavigation: React.FC<TabNavigationProps> = ({
	tabs,
	onFilter,
	showFilter = true,
			...
			<AnimatePresence mode="wait" initial={false}>
				{tabs.map((tab) => (
					<TabsContent
						key={tab.id}
						value={tab.id}
						className="absolute inset-0 flex flex-col gap-[34px] w-full h-full overflow-hidden"
						asChild
					>
						<motion.div
							initial={{ opacity: 0 }}
                ...
                <Tabs value={activeTabId} onValueChange={setActiveTabId} className='flex flex-col w-full h-full'>
                    <div>
                        <div className='flex items-center justify-between'>
                            ...
                            <TabsList className='inline-flex h-[60px] border-b-[1px] border-b-primary-200 p-0 rounded-t-[10px] bg-white w-full'>
                                {tabs.map((tab) => (
                                    <TabsTrigger
                                        key={tab.id}
                                        value={tab.id}
                                        className='relative h-full rounded-none border-none bg-transparent w-full px-6 py-4 text-base font-medium text-text-300 hover:text-text-500 data-[state=active]:bg-transparent data-[state=active]:text-text-800 data-[state=active]:font-bold data-[state=active]:shadow-none transition-colors'>
                                        {tab.label}
                                        {activeTabId === tab.id && (
                                            <motion.div
                                                layoutId='pageTabUnderline'
                                                initial={{ opacity: 0.8 }}
                                                animate={{ opacity: 1 }}
                                                transition={{ duration: 0.2, ease: 'easeInOut' }}
                                                className='absolute -bottom-px left-0 right-0 h-[3px] bg-primary-700'
                                            />
                                        )}
                                    </TabsTrigger>
                                ))}
                    ...
                    {/* Tab content area */}
                    <div className={cn('flex flex-grow overflow-hidden')}>
                        <motion.div
                            layout
                            transition={{ duration: 0.2, ease: 'easeOut' }}
                            className='relative flex flex-grow w-full overflow-hidden'>
                            {tabContent}
                        </motion.div>
                        {buildPanelContent}
                    </div>
